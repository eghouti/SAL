...
class ShiftAttention(torch.nn.Conv2d):
    ...
    def __init__(self, in_features, out_features, stride=1, bias=False, padding = 0, kernel_size=3):
        ...
        self.attentionWeights = torch.nn.parameter.Parameter(torch.FloatTensor(1, in_features, kernel_size* kernel_size))
        torch.nn.init.uniform_(self.attentionWeights)

    def forward(self,input):
        attention = self.attentionWeights / self.attentionWeights.std(dim=2).view(1, self.in_features, -1)
        ...
        attention = attention.view(1, self.in_features, self.kernel_width, self.kernel_width)
